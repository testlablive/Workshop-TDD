...
export const HomePage: React.FC = () => {
  ...
  const { joinSession, createSession, error } = useSession();
  ...
  const handleJoinSession = async (e: React.FormEvent) => {
    ...
    if (joinSessionId && joinName) {
      try {
        setFormError(null);
        await joinSession(joinSessionId, joinName, joinRole);
  ...
  const handleCreateSession = async (e: React.FormEvent) => {
    ...
    if (createName && hostName) {
      try {
        setFormError(null);
        const sessionId = await createSession(createName, createDescription);
        await joinSession(sessionId, hostName, 'player');
  ...
  return (
    <div className={styles.container}>
      ...
      <Card className={styles.card}>
        <CardHeader header={<Text weight="semibold" size={500}>Join Existing Session</Text>} />
        ...
        <form onSubmit={handleJoinSession} className={styles.form}>
          ...
          <Button appearance="primary" type="submit" disabled={!joinSessionId || !joinName}>
            Join Session
          </Button>
        </form>
        ...
        {loadingSessions ? (
          ...
          <div className={styles.sessionsTable}>
            <Text weight="semibold" size={300}>Available Sessions</Text>
            <Table>
              <TableHeader>
                <TableRow>
        ...
        <CardHeader header={<Text weight="semibold" size={500}>Create New Session</Text>} />
        ...
        <form onSubmit={handleCreateSession} className={styles.form}>
          ...
          <Button appearance="primary" type="submit" disabled={!createName || !hostName}>
            Create Session
          </Button>
        </form>
        
        {(error || formError) && <Text className={styles.error}>{formError || error}</Text>}
      </Card>
    </div>
  );
};