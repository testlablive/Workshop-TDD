...
export const mockGetSession = async (sessionId: string): Promise<Session> => {
  ...
  if (session.currentStory) {
    ...
    const currentStory = sessionStories.find(s => s.id === session.currentStory?.id);
    if (currentStory) {
...
export const mockCreateSession = async (session: Omit<Session, 'id'>): Promise<Session> => {
  await delay(500);
...
export const mockCreateStory = async (sessionId: string, story: Omit<Story, 'id'>): Promise<Story> => {
  await delay(500);
...
export const mockUpdateStory = async (sessionId: string, storyId: string, storyUpdate: Partial<Story>): Promise<Story> => {
  await delay(500);
  const sessionStories = stories[sessionId];
  if (!sessionStories) {
    throw new Error('Session not found');
...
export const mockJoinSession = async (sessionId: string, user: Omit<User, 'id'>): Promise<User> => {
  await delay(500);
  const session = sessions.find(s => s.id === sessionId);
  if (!session) {
    throw new Error('Session not found');
...
export const mockSubmitVote = async (sessionId: string, storyId: string, vote: Omit<Vote, 'id'>): Promise<Vote> => {
  await delay(500);
  const sessionStories = stories[sessionId];
  if (!sessionStories) {
    throw new Error('Session not found');
...
export const mockRevealVotes = async (sessionId: string, storyId: string): Promise<Story> => {
  await delay(500);
  const sessionStories = stories[sessionId];
  if (!sessionStories) {
    throw new Error('Session not found');
...
export const mockResetVotes = async (sessionId: string, storyId: string): Promise<Story> => {
  await delay(500);
  const sessionStories = stories[sessionId];
  if (!sessionStories) {
    throw new Error('Session not found');
...
export const mockFinalizeEstimate = async (sessionId: string, storyId: string, estimate: string): Promise<Story> => {
  await delay(500);
  const sessionStories = stories[sessionId];
  if (!sessionStories) {
    throw new Error('Session not found');
